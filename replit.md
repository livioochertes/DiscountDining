...
- **Real-Time Notification System**: Dual-layer notification system: SSE (foreground) + Push (background). Server: `server/sseNotifications.ts` with authenticated SSE endpoint (`GET /api/restaurant/:id/notifications/stream`, requires `requireAuth`), multi-restaurant support (owner connects once, receives events for all owned restaurants), `notifyRestaurant()` function called from reservation/order creation and status change endpoints. `server/pushNotifications.ts` with token registration (`POST /api/push/register`), unregistration (`DELETE /api/push/unregister`), Firebase Admin SDK integration (graceful fallback when not configured), auto-cleanup of invalid tokens. DB: `device_push_tokens` table. **Mobile client**: `MobileRestaurantDashboard.tsx` connects via `EventSource` for SSE, uses `@capacitor/push-notifications` on native for push token registration and foreground notification handling. Visual: animated alert banners (blue=reservations, orange=orders), Web Audio API notification sounds (reusable AudioContext), tab badges with counts. Sound toggle persisted in localStorage. **Web portal client**: `RestaurantNotificationSystem.tsx` uses SSE (EventSource) for instant notifications — replaced 10s polling. Reuses single AudioContext, shows popup cards with View Details/Dismiss, bell badge with unread count. Push notifications require `FIREBASE_SERVICE_ACCOUNT` env var and `google-services.json` for Android / APNs config for iOS.
