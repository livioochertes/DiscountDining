...
- **Real-Time Notification System**: SSE (Server-Sent Events) for instant restaurant notifications. Server: `server/sseNotifications.ts` with authenticated SSE endpoint (`GET /api/restaurant/:id/notifications/stream`, requires `requireAuth`), `notifyRestaurant()` function called from reservation/order creation and status change endpoints. Client: `MobileRestaurantDashboard.tsx` connects via `EventSource`, invalidates React Query caches on events (`new_reservation`, `new_order`, `reservation_updated`, `order_updated`), shows animated alert banners (blue for reservations, orange for orders), plays Web Audio API notification sounds (reusable AudioContext), tab badges with counts on Reservations/Orders tabs. Sound toggle persisted in localStorage.
