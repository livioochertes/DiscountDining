import { pgTable, text, serial, integer, boolean, decimal, timestamp, varchar, index, jsonb, date } from "drizzle-orm/pg-core";
...
export const crmPlans = pgTable("crm_plans", {
  id: serial("id").primaryKey(),
  name: varchar("name", { length: 50 }).notNull(),
  slug: varchar("slug", { length: 50 }).notNull().unique(),
  monthlyPriceEur: decimal("monthly_price_eur", { precision: 10, scale: 2 }).notNull(),
  yearlyPriceEur: decimal("yearly_price_eur", { precision: 10, scale: 2 }).notNull(),
  stripePriceIdMonthly: varchar("stripe_price_id_monthly", { length: 255 }),
  stripePriceIdYearly: varchar("stripe_price_id_yearly", { length: 255 }),
  features: jsonb("features").notNull().default('[]'),
  maxCustomers: integer("max_customers").notNull().default(50),
  maxCampaignsPerMonth: integer("max_campaigns_per_month").notNull().default(0),
  maxSegments: integer("max_segments").notNull().default(0),
  isActive: boolean("is_active").default(true).notNull(),
});

export const insertCrmPlanSchema = createInsertSchema(crmPlans).omit({ id: true });
export type CrmPlan = typeof crmPlans.$inferSelect;
export type InsertCrmPlan = z.infer<typeof insertCrmPlanSchema>;
...
export const insertCrmSubscriptionSchema = createInsertSchema(crmSubscriptions).omit({ id: true, createdAt: true, updatedAt: true });
...
export const customerSegments = pgTable("customer_segments", {
  ...
  segmentType: varchar("segment_type", { length: 20 }).notNull().default("custom"),
...
export const customerSegmentMembers = pgTable("customer_segment_members", {
  id: serial("id").primaryKey(),
  segmentId: integer("segment_id").notNull().references(() => customerSegments.id),
  customerId: integer("customer_id").notNull().references(() => customers.id),
  addedAt: timestamp("added_at").defaultNow().notNull(),
});

export const insertCustomerSegmentMemberSchema = createInsertSchema(customerSegmentMembers).omit({ id: true, addedAt: true });
...
export const insertCrmCampaignSchema = createInsertSchema(crmCampaigns).omit({ id: true, createdAt: true });
...
export const crmCampaignRecipients = pgTable("crm_campaign_recipients", {
  id: serial("id").primaryKey(),
  campaignId: integer("campaign_id").notNull().references(() => crmCampaigns.id),
  customerId: integer("customer_id").notNull().references(() => customers.id),
  status: varchar("status", { length: 20 }).notNull().default("pending"),
  sentAt: timestamp("sent_at"),
});

export const insertCrmCampaignRecipientSchema = createInsertSchema(crmCampaignRecipients).omit({ id: true });
...
export const customerSpecialDates = pgTable("customer_special_dates", {
  id: serial("id").primaryKey(),
  customerId: integer("customer_id").notNull().references(() => customers.id),
  restaurantId: integer("restaurant_id").notNull().references(() => restaurants.id),
  dateType: varchar("date_type", { length: 20 }).notNull(),
  eventDate: date("event_date").notNull(),
  label: varchar("label", { length: 100 }),
  createdAt: timestamp("created_at").defaultNow().notNull(),
});

export const insertCustomerSpecialDateSchema = createInsertSchema(customerSpecialDates).omit({ id: true, createdAt: true });
export type CustomerSpecialDate = typeof customerSpecialDates.$inferSelect;
export type InsertCustomerSpecialDate = z.infer<typeof insertCustomerSpecialDateSchema>;

export const customerCrmNotes = pgTable("customer_crm_notes", {
  id: serial("id").primaryKey(),
  customerId: integer("customer_id").notNull().references(() => customers.id),
  restaurantId: integer("restaurant_id").notNull().references(() => restaurants.id),
  ownerId: integer("owner_id").notNull().references(() => restaurantOwners.id),
...
export const insertCustomerCrmNoteSchema = createInsertSchema(customerCrmNotes).omit({ id: true, createdAt: true });
export type CustomerCrmNote = typeof customerCrmNotes.$inferSelect;
export type InsertCustomerCrmNote = z.infer<typeof insertCustomerCrmNoteSchema>;
...
export const crmAutomations = pgTable("crm_automations", {
  ...
  inactivityDays: integer("inactivity_days").default(60).notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
});

export const insertCrmAutomationSchema = createInsertSchema(crmAutomations).omit({ id: true, createdAt: true });
